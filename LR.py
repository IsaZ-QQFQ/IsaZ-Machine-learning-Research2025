...
def simple_inverse_prediction(y_tar, pipeline, x_feature, x_template):
    def objective(x_value):
        x_temp = x_template.copy()
        x_temp[x_feature] = x_value
        y_pre = pipeline.predict(x_temp)[0]
        return (y_pre - y_tar) ** 2

    bounds = (x_template[x_feature].min(), x_template[x_feature].max())
    result = minimize(objective, x0=x_template[x_feature].mean(), bounds=[bounds])
    return result.x[0]
